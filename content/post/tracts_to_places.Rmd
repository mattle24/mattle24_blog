---
title: "Matching Tracts to Cities"
author: "Matt Lehman"
date: 2019-02-12
categories: ["R"]
...
As is often the case, all it took was learning about the right package. The [`sf`](https://github.com/r-spatial/sf) package provided me with all of the tools I needed, given shapefiles of both the tracts and places. In my research, I used data from [IMPUS NHGIS](nhgis.org), which has been an invaluable resource during this project. However, to keep this example easily accessible, I will instead use tract data from the American Community Survey via [`tidycensus`](https://github.com/walkerke/tidycensus) and place boundaries from [`tigris`](https://github.com/walkerke/tigris). Using `tidycensus` does require requesting a free API key, which you can do [here](https://api.census.gov/data/key_signup.html). 
...
The tracts are in tidy format, with one row for variable `B02001_002E` (population white) and another row for `B02001_003E` (populaiton black/ Africa American) but I want both variables in one row per tract. 
...
To simplify the example, I'm going to isolate one city to see how tracts and places match. Since my research is focused on Indianapolis, I will use that as the example. First, let's look at Indianapolis by itself.
...
Now, let's match the tracts. First, I'll try `st_covers()` (a helpful [`sf` cheatsheet is available](https://github.com/rstudio/cheatsheets/blob/master/sf.pdf) for these functions). 
...
`st_covers()` does an okay job, but I'm missing all of the tracts with area outside of the place Indianapolis. Next, I tried `st_intersects()`.
...
`st_intersects()` was better, but still problematic. It picked up all of the tracts inside of Indianapolis, which is great, but also all of the tracts bordering Indianapolis. To rememedy this, I used `st_intersection()`. Unlike the previous two functions, `st_intersection()` returns an `sf` object. To simplify, I'm going to use the subset of the tracts defined by `st_intersects()` instead of the full `ind_tracts`.
...
`st_intersection()` seems to essentially trim the tracts to make sure they are within Indianapolis. However, the new `sf` object contains all of the tracts as the `sf` object indexed by `intersects_matrix`.
...
Using the percents as weights, I can now calculate the dissimilarity index. 
...
dissimilarity_index <- function(sub_geometry, majority_pop = white_pop, minority_pop = black_pop) {
  ...
  sub_geometry %>%
    mutate(
      ...
      ,T. = sum(t) # total pop in macro geometry
      ,p = !!minority_pop / t # prop minority in tract
      ,P. = sum(!!minority_pop) / T. # prop minority in macro geometry
    ) %>% 
    summarise(
      dissimilarity_index = sum(wt * t * abs(p - P.)) / mean(2 * T. * P. * (1 - P.))
      ) %>% 
    .$dissimilarity_index
}
